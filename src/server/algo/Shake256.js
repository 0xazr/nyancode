class Shake256 {
  static hash(message, outputLength) {
    ...
    // Squeezing phase
    const output = this.squeeze(state, outputLength);
  ...
  static padMessage(message, rate) {
    ...
    paddedMessage[message.length] = 0x1f; // Append suffix byte
  ...
  static squeeze(state, outputLength) {
    const blockSize = state.length;
    const output = new Uint8Array(outputLength);
    let outputOffset = 0;

    while (outputOffset < outputLength) {
      const block = state.subarray(0, blockSize);
