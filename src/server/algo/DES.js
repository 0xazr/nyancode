...
class DES {
  ...
  fCalculation(subkeyi, ebits) {
    ...
    for (let i = 0; i < 8; i++) {
      let xorEbSk = Number(
        parseInt(ebitSeq[i], 2) ^ parseInt(subkeySeq[i], 2)
      ).toString(2);
    ...
    for (let i = 0; i < 8; i++) {
      let _4bit = Number(
        eval(`this.S${i + 1}`)[positionsArr[i][0]][positionsArr[i][1]]
      ).toString(2);
  ...
  DESEncryptionRound(_leftPart, _rightPart, subkey) {
    ...
    for (let i = 0; i < 8; i++) {
      let xorEbSk = Number(
        parseInt(leftSeq[i], 2) ^ parseInt(fResultSeq[i], 2)
      ).toString(2);
